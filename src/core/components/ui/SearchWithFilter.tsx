      ...
      <div className={`flex gap-3 ${className}`} dir={isRTL ? 'rtl' : 'ltr'}>
        <div className={`relative flex-none w-[373px] ${isRTL ? 'order-1' : 'order-1'}`}>
          ...
          <input
            ...
            className={`w-full py-2 bg-background border border-input text-foreground placeholder:text-muted-foreground rounded h-10 outline-none focus:outline-none focus:border-ring focus:ring-2 focus:ring-ring ${isRTL ? 'pr-10 pl-4 text-right' : 'pl-10 pr-4'}`}
          />
        </div>
        ...
        {showSortButton && (
          <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className={`bg-background border-input text-foreground hover:bg-accent rounded h-10 shadow-none font-poppins font-normal text-base transition-all duration-200 ${isRTL ? 'order-2' : 'order-2'}`}
              >
                ...
                <div className="space-y-0">
                    {sortOptions.map((option) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 text-base transition-all duration-200 flex items-center gap-3 hover:bg-accent ${
